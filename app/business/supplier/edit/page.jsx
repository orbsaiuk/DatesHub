...
export default async function SupplierEditPage() {
  ...
  const supplier = await getSupplierEditData(tenantId);

  if (!userId) {
    redirect("/sign-in?redirect_url=/business/supplier/edit");
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8 px-4 sm:px-6 md:px-8">
      ...
      {/* Two-column layout */}
      <EditPageClient initialEntity={supplier} entityType="supplier" />
    </div>
  );
}
