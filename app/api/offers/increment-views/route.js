...
export async function POST(req) {
  try {
    ...
    const found = await writeClient.fetch(
      `*[_type == "offer" && _id == $id][0]{ _id, _rev, tenantId, totalViews }`,
      { id }
    );
    if (!found?._id) {
      return NextResponse.json(
    ...
    // Skip offer owners (members of the tenant that owns the offer)
    const { userId } = await auth();
    if (userId && found.tenantId) {
      try {
        // Check if user is a member of the company/supplier that owns this offer
    ...
    // Deduplication key: offer + ipHash + day
    const day = new Date().toISOString().slice(0, 10);
    const ipKey = hashIp(ip);
    const dedupeKey = `${found._id}:${day}:${ipKey}`;

    // Already viewed? Check in Sanity "views" array
    const alreadyViewed = await writeClient.fetch(
      `*[_type == "offer" && _id == $offerId && "${dedupeKey}" in views[].key][0]{ _id }`,
      { offerId: found._id }
    );
    ...
    console.error("offer increment-views error", err);
