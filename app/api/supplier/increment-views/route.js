...
import { USER_SUPPLIER_MEMBERSHIPS_QUERY } from "@/sanity/queries/supplier";
...
export async function POST(req) {
  try {
    ...
    const found = await writeClient.fetch(
      `*[_type == "supplier" && (tenantId == $id || slug.current == $id)][0]{ _id, _rev, tenantId, totalViews }`,
      { id }
    );
    if (!found?._id) {
      return NextResponse.json(
    ...
    const alreadyViewed = await writeClient.fetch(
      `*[_type == "supplier" && _id == $supplierId && "${dedupeKey}" in views[].key][0]{ _id }`,
    ...
    console.error("supplier increment-views error", err);
