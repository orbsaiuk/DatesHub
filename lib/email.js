...
export function buildBasicHtmlEmail(title, bodyLines) {
  ...
  const p = safeLines
    ...
    .join("");
  return (
    `<!doctype html><html dir="rtl"><body style="font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica,Arial,sans-serif; padding: 16px; direction: rtl;">` +
    `<h2 style="margin:0 0 12px;">${escapeHtml(title || "")}</h2>` +
    p +
    `<p style="color:#6b7280; font-size:12px; margin-top:24px;">هذه رسالة تلقائية. يرجى عدم الرد عليها.</p>` +
    `</body></html>`
  );
}
